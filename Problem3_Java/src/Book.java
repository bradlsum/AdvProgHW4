...
abstract public class Book {
    ...
    public String toString(){
        return ("A book called '" + title +
                ...
                "at " + this.locationCode);
    }

    public String authorList(){
        String temp = "";
        if (this.authors.size() == 1) return this.authors.toArray(new Author[this.authors.size()])[0].getName();
        for (int i = 0; i < this.authors.size() - 1; ++i){
            temp += this.authors.toArray(new Author[this.authors.size()])[i].getName() + ", ";
        }
        temp += this.authors.toArray(new Author[this.authors.size()])[this.authors.size() - 1].getName();
        return temp;
    }
    ...
    public void removeAuthor(String a){
        for (int i = 0; i < this.authors.size(); ++i){
            if (this.authors.toArray(new Author[this.authors.size()])[i].getName() == a) this.authors.remove(this.authors.toArray(new Author[this.authors.size()])[i]);
        }
    }
}
